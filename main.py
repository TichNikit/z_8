...
def main():
    """
    Основная функция программы, которая управляет потоком выполнения.
    Запрашивает у пользователя тикер акций, период, даты и стиль графика.
    Загружает данные акций, добавляет скользящую среднюю и вычисляет статистику.
    Уведомляет о сильных колебаниях цены акций и экспортирует данные в CSV.
    Строит графики цен акций и отображает интерактивный график.
    """
